...
__all__ = ['sysinfo', 'mkdir_if_needed', 'get_data', 'get_weights', 'meta_to_img_path', 'meta_to_mask_path',
...
# Cell
def get_weights(
        model,                  # you've already defined your model; we're just loading weights here
        weights_type='segreg',  # 'segreg', 'rings', 'bboxes' . trained on real only
        force_download=False,      # re-downloads even if you've already got one
        dest_root='~/.espiownage/models', # where the model gets saved
        ):
    "Grab weights of pretrained model"
    assert weights_type in ['segreg', 'rings', 'bboxes']
    urls = {}
    urls['segreg']  = ''
    urls['rings'] = ''
    urls['bboxes']    = ''
    if 'bboxes' == weights_type:
        print("Note that the IceVision install is incompatible with the fastai version used for the seg-reg and ring counting")
        print("For bounding boxes you (probably) need a different Python environment.")
